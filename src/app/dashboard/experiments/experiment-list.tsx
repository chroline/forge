...
export function ExperimentList({
  ...
  const handleSelectionChange = (experimentId: string, selected: boolean) => {
    const newSelectedIds = selected
      ? [...selectedIds, experimentId]
      : selectedIds.filter((id) => id !== experimentId);
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 space-y-4 pt-4 overflow-y-auto">
        {experiments.map((experiment) => {
          const isActive =
            pathname === `/dashboard/experiments/${experiment.id}`;
